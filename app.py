...
def load_models():
    ...
    try:
        ...
        models["autoencoder"] = load_model("autoencoder_final.h5")
    except Exception as e:
        errors.append(f"Autoencoder: {e}")

    try:
        from stable_baselines3 import PPO
        models["rl"] = PPO.load("rl_model_final")
    except Exception as e:
...
with tab2:
    ...
    if uploaded:
        ...
        if res:
            ...
            st.plotly_chart(mse_chart(m_err, 0.05), use_container_width=True, key="live_mse")
...
with tab5:
    ...
    # CIC-IDS2017 top features used in most published models
    FEATURE_NAMES = [
        "Flow Duration", "Total Fwd Packets", "Total Bwd Packets",
        "Total Length Fwd", "Total Length Bwd",
    ...
    with sec1:
        ...
        KEY_FEATURES = [
            ...
            ("Total Bwd Packets",     0.0,   5000, 5.0),
